...
function iSlider(opts) {
  this.opts = {
    ...
    scroolItem: null,  // 修复禁止下滑且可以上滑bug
    onslide: function (index) {
    },//滑动回调 参数是本对象
    onloading: function (loaded, total) {
    },
...
iSlider.prototype = {
  ...
  init: function () {
    ...
    // this._tpl = $(this.wrap).clone(true)[0];
    this._tpl = this.wrap;
  ...
  _setHTML: function (index) {
    ...
    if (this.index > 0) {
      // this._prev = $(this._tpl[this.index - 1]).clone(true)[0];
      this._prev = this._tpl[this.index - 1];
      this._prev.style.cssText += this._getTransform('-' + this.scrollDist + 'px');
      initDom.appendChild(this._prev)
    } else {
      this._prev = null;
    }
    // this._current = $(this._tpl[this.index]).clone(true)[0];
    this._current = this._tpl[this.index];

    this._current.style.cssText += this._getTransform(0);
    initDom.appendChild(this._current);

    if (this.index < this.length - 1) {
      // this._next = $(this._tpl[this.index + 1]).clone(true)[0];
      this._next = this._tpl[this.index + 1];
...
//                console.info(e)
  ...
  _touchmove: function (e) {
    ...
    if (this.opts.noslide && this.opts.noslide.indexOf(this.index) >= 0) {
      var scrollTop = $('.box').eq(1).scrollTop();
      //noslideBack 默认值是false   默认是禁用滑动后 前后都不能再滑动,
      //但是当noslideBack为true时, 禁用了这一页的滑动, 那么往下是划不动了  但是可以往上滑
      if (!!this.opts.noslideBack && this.opts.scroolItem) {
        if (this.opts.isVertical) {
          if (e.touches[0].pageY - this._touchstartY < 0) {
            return;
          } else if (scrollTop > 0) {
            return;
          }
          ...
          if (e.touches[0].pageX - this._touchstartX < 0) {
            return;
          }
        }
...
//                console.info(e)
      ...
      // self._prev = $(self._tpl[prevIndex]).clone(true)[0];
      self._prev = self._tpl[prevIndex];
...
//                console.info(e)
      ...
      // self._next = $(self._tpl[nextIndex]).clone(true)[0];
      self._next = self._tpl[nextIndex];
